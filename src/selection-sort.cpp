...
//     [ A | C D E F (B) Q R S ]
//     [ A B | D E F C Q R S ]
// Note that we conduct ~(n^2 / 2 compares + n swaps) to create an ascending sort
// of comparable items in time O(n^2).
...
int main (int argc, char* argv[])
{
    ...
    if (!sel.is_sorted(tokens)) {
        cout << "ERROR: upon review, selection sort failed to completely sort the data." << endl; 
        return EXIT_FAILURE;
    }
