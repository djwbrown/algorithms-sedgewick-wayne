...
// NOTES
// Insertion sort works sequentially from the beginning of the array until it
// encounters an element which is not properly sorted compared to the previous
// element (F is not < D) .
//     [ A B C E F | (D) Q R S ]
// Swap up through the previous entries until we find the appropriate insertion
// index.
//     [ A B C E (D) F Q R S ]
//     [ A B C D E F | Q R S ]
// Continue from the index | shown above. Note that we conduct ~(n^2 / 4
// compares and swaps) on average (worst case n^2 / 2) to create an ascending
...
int main(int argc, char *argv[]) {
  ...
  if (!ins.is_sorted(tokens)) {
    cout << "ERROR: upon review, insertion sort failed to completely sort "
            "the data."
         << endl;
    return EXIT_FAILURE;
  }
