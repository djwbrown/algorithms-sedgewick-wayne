...
// accessed individually but are emptied all at once (and in an undefined
// order).
...
template <typename T> string Bag<T>::print_contents() {
  ...
  // far less prone to bugs. See `lifo-stack-resizing-array.cpp` for an
  // iterator.
  string str;
  str.reserve(size() * 5);
...
int main() {
  ...
  cout << "Created an empty multiset for double type items, with a linked "
          "list implementation."
       << endl;
  ...
  test_multiset.add(1.01);
  cout << "Request status, the multiset has size: " << test_multiset.size()
       << endl;
  cout << "Request status, the multiset is empty: "
       << (test_multiset.is_empty() ? "true" : "false") << endl;

  // Use the member functions to print the contents and empty the bag. Happens
  // to be in LIFO order.
  cout << "Bag contains these items: {" << test_multiset.print_contents()
       << " }" << endl;
