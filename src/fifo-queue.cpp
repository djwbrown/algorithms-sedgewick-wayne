...
// NOTES
// Once again, users of this class would be better served by the standard library with
// std::deque (double-ended queue). Unlike std::vector, insertions and deletions at either
// end of a std::deque are constant-time O(1). Use the push_front() and pop_back() member functions.
...
int main() {
	...
	cout << "Created an empty FIFO queue for double type items, with a linked list implementation." << endl;
	...
	cout << "Request status, the FIFO queue has size: " << fifo_queue.size() << endl;
	cout << "Request status, the FIFO queue is empty: " << (fifo_queue.is_empty() ? "true" : "false") << endl;
